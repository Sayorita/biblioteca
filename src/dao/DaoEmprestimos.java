    ...
    public void registrarEmprestimo(Emprestimos emt)
{
    ...
    try
    {
        ...
        pstm = con.prepareStatement("INSERT INTO emprestimso(nome,nCadastro,codLivro,titulo,dataEmpre,dataDevo) VALUES (?,?,?,?,?,?)");
        pstm.setString(1, emt.nome);
        pstm.setInt(2, emt.nCadastro);
        pstm.setInt(3, emt.codLivro);
        pstm.setString(4, emt.titulo);
        pstm.setInt(5, emt.dataEmpre);
        pstm.setInt(6, emt.dataDevo);
        pstm.execute();
...
public List<Emprestimos> getEmprestimos()
 {
     ...
     if(rs.first())
     {
         do{
            ...
            ep.dataEmpre = rs.getInt("dataEmpre");
            ep.dataDevo = rs.getInt("dataDevo");
