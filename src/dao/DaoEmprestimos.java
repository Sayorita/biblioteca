    ...
    public void registrarEmprestimo(Emprestimos emt)
{
    ...
    try
    {
        ...
        pstm = con.prepareStatement("INSERT INTO emprestimos(nome,nCadastro,codLivro,titulo,dataEmpre,dataDevo,status) VALUES (?,?,?,?,?,?,?)");
...
public void alterarEmprestimo(Emprestimos emt)
{
    ...
    try
    {
        ...
        pstm = con.prepareStatement("UPDATE emprestimos SET nome=?,nCadastro=?,codLivro=?,titulo=?,dataEmpre=?,dataDevo=?,status=? WHERE idEmprestimo=?");
        pstm.setString(1, emt.nome);
        pstm.setInt(2, emt.nCadastro);
        pstm.setInt(3, emt.codLivro);
        pstm.setString(4, emt.titulo);
